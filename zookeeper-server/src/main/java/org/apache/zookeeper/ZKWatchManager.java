...
class ZKWatchManager implements ClientWatchManager {
    ...
    /**
     * {@link org.apache.zookeeper.ZooKeeper.DataWatchRegistration}
     * <br>节点必须存在, 触发时机: 节点被删除 OR 数据变更
     */
    private final Map<String, Set<Watcher>> dataWatches = new HashMap<>();  // getData、getConfig
    /**
     * {@link org.apache.zookeeper.ZooKeeper.ChildWatchRegistration}
     * <br>节点必须存在, 触发时机: 节点被删除 + 子节点被创建 OR 删除
     */
    private final Map<String, Set<Watcher>> childWatches = new HashMap<>(); // getChildren
    /**
     * {@link org.apache.zookeeper.ZooKeeper.ExistsWatchRegistration}
     * <br>节点可以不存在, 触发时机: 节点被创建 OR 删除 OR 数据变更
     */
    private final Map<String, Set<Watcher>> existWatches = new HashMap<>(); // exists
    /**
     * {@link org.apache.zookeeper.ZooKeeper.AddWatchRegistration}
     * <br>持久化, 节点可以不存在, 触发时机: 节点被创建 OR 删除 OR 数据变更
     */
    private final Map<String, Set<Watcher>> persistentWatches = new HashMap<>();          // addWatch + AddWatchMode.PERSISTENT
    /**
     * {@link org.apache.zookeeper.ZooKeeper.AddWatchRegistration}
     * <br>持久化递归, 节点可以不存在, 触发时机: 节点被创建 OR 删除 OR 数据变更
     */
    private final Map<String, Set<Watcher>> persistentRecursiveWatches = new HashMap<>(); // addWatch + AddWatchMode.PERSISTENT_RECURSIVE
